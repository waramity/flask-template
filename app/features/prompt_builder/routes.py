...
def submit_edit_prompt(profile_name, slug, category_slug):
    if request.method == 'PATCH' and request.json is not None:
        ...
        prompt_json = {"$set": {"categories.$[cat].prompts.$[pro]": {"name": request.json['prompt']['name'], "slug": request.json['prompt']['slug'], "image_url": cdn_url}}}
        feature_db.prompt_builder.update_one(
          { '_id': prompt_builder['_id'], 'user_id': prompt_builder_creator['_id'] },
          prompt_json,
        ...
        return make_response(jsonify({'status': 1, 'prompt': {'name': request.json['prompt']['name'], 'image_url': cdn_url, 'slug': request.json['prompt']['slug']}}), 200)
